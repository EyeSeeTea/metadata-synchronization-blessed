...
type SynchronizationBuilder = unknown;
...
export default async function migrate(api: D2Api): Promise<void> {
    ...
    const newRules: SynchronizationRuleNew[] = oldRules.map(ins => _.omit(ins, ["builder"]));
    ...
    await promiseMap(oldRules, async oldRule => {
        const newRuleDetails: Maybe<SynchronizationRuleDetailsNew> = { builder: oldRule.builder };
